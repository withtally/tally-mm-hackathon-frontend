...
export const useListOwnedVaults = (): Values => {
  ...
  const getVaultsOfAddress = async (address: string): Promise<Vault[]> => {
    ...
    if (addressNFTBalance.toNumber() === 0) {
      ...
      for (let vaultId = 0; vaultId <= nftCurrentSupply; vaultId++) {
        ...
        const vaultEpochExpiry = (await vaultFactory?.vaultToExpiryEpochs(vaultId));
        ...
        if (ownerAddress === address) {
          ...
          result.push({
            vaultId,
            vaultAddress,
            vaultBalance: parseBigNumber(Number.parseInt(vaultBalance)),
            vaultEpochExpiry: vaultEpochExpiry.toString(),
          });
        }
      }
    }
